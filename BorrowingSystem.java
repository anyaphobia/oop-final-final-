...
public class BorrowingSystem{ 
    ...
    public BorrowingSystem(ArrayList<Material> materials, UserManager userManager, ArrayList<BorrowerHistory> borrowerHistory, ArrayList<LiteraryHistory> literaryHistory){ 
    ...
    public void BorrowProcess(){
        ...
        if (chosen == null){
            System.out.println("Material with ID " + materialId + " cannot be found.");
            return;
        }
    ...
    public void borrowMaterial (Person user, Borrower borrower, Material material){
        ...
        try {
            ...
            System.out.println("Borrower Name:" + borrower.fullName);
            System.out.println("Borrowed Material Title: " + material.getTitleOrName());
            System.out.println("Due Date:" + borrower.dueDate);

            String date = java.time.LocalDate.now().toString();
            borrowerHistory.add(new BorrowerHistory(borrower.fullName, material.getTitleOrName(), "BORROWED", date));
            literaryHistory.add(new LiteraryHistory(borrower.fullName, material.getTitleOrName(), "BORROWED", date));
        }
        catch (Exception e) {
    ...
    public String getdueDate(String materialType){
        ...
        switch (materialType) {
            ...
            default:
                return "Unknown material type. Please enter only the material type available on this library type.";
        }
    }
